...
async function extractJobs() {
  ...
  if (!tab.url.includes('energyjobsearch.com')) {
    showMessage('Please navigate to EnergyJobSearch.com first', true);
    return;
  }
  ...
  try {
    await chrome.scripting.executeScript({
      target: { tabId: tab.id },
      func: () => {
        if (window.EJS && window.EJS.extract) {
          window.EJS.extract();
          return true;
        }
        return false;
...
async function checkAvailability() {
  ...
  try {
    await chrome.scripting.executeScript({
      target: { tabId: tab.id },
      func: () => {
        if (window.EJS && window.EJS.checkAvailability) {
          window.EJS.checkAvailability();
          return true;
        }
        return false;
      }
    });
    showMessage('Starting availability check...');
  } catch (e) {
    showMessage('Error: ' + e.message, true);
  }
}
...
async function startCrawl() {
  ...
  if (!tab.url.includes('energyjobsearch.com')) {
    showMessage('Please navigate to EnergyJobSearch.com search results first', true);
    return;
  }
  ...
  try {
    await chrome.scripting.executeScript({
      target: { tabId: tab.id },
      func: (delaySeconds) => {
        if (window.EJS && window.EJS.crawl) {
          window.EJS.crawl(delaySeconds, 20);
          return true;
        }
        return false;
      },
...
async function startAutoFetch() {
  ...
  try {
    await chrome.scripting.executeScript({
      target: { tabId: tab.id },
      func: (delaySeconds) => {
        if (window.EJS && window.EJS.autoFetch) {
          window.EJS.autoFetch(delaySeconds);
          return true;
        }
        return false;
      },
